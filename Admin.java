...
public class Admin {
            ...
            if (i == 1) {
                ...
                if (test > 0) {//si groupe existe 
                    ...
                    if ((val == 3 || val == 0) && (j != 1)) {
                        ...
                        if (testcapacite(idseance, listgrp) == true) {
                            ...
                            System.out.println("salle complete");
                        }
    ...
    public void ajoutseeance(String date, String hd, int etat, int idcours, int idtype, ArrayList<Integer> idprof, ArrayList<Integer> idgroupe, ArrayList<Integer> idsalle) {
    ...
    public void deplacerceance(String date, String hd, int idseance) {
        try {
            int i = 0;
            int val = 0;
            int j = 0;
    ...
    public void ajoutesalle(int idseance, int idsalle) {
        String date = "";
        String hd = "";
        int val = 0;
        try {
    ...
    public boolean testcapacite(int idseance, ArrayList<Integer> idgrp) {
        try {
            int cap = 0;
            int max = 0;

            PreparedStatement stmt = connect.prepareStatement("SELECT * FROM seance_salle WHERE ID_SEANCE=?");
            stmt.setInt(1, idseance);
            //stmt1.setInt(2,idgroupe);

            ResultSet rs = stmt.executeQuery();////cherche groupe affcter a la seance
            while (rs.next()) {
                PreparedStatement stmt2 = connect.prepareStatement("SELECT * FROM salle WHERE ID=?");
                stmt2.setInt(1, rs.getInt("ID_SALLE"));
                //stmt1.setInt(2,idgroupe);
                ResultSet rs2 = stmt2.executeQuery();
                while (rs2.next()) {
                    max = rs2.getInt("CAPACITE");
                }
